...
var _renderSelectedStoreList = _interopRequireDefault(require("./renderSelectedStoreList.js"));
...
var makeVoteList = function makeVoteList(params) {
          ...
          switch (_context.prev = _context.next) {
            ...
            case 2:
              ...
              selectedStore = {
                ...
                thumbnails: _toConsumableArray($store.querySelectorAll('.store-image')).map(function ($store) {
                  return $store.style.backgroundImage.slice(4, -1).replace(/"/g, '');
                }),
                x: $store.querySelector('.x').textContent,
                y: $store.querySelector('.y').textContent
              };
              selectedStoreList = [].concat(_toConsumableArray(selectedStoreList), [selectedStore]);
              e.target.disabled = true;
              (0, _renderSelectedStoreList["default"])(selectedStoreList);
            case 7:
            case "end":
  ...
  window.addEventListener('click', /*#__PURE__*/function () {
    var _ref2 = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2(e) {
      var targetId, $addStore, _firebase$firestore$F, voteItem;
          ...
          switch (_context2.prev = _context2.next) {
            case 0:
              ...
              if (e.target.matches('.remove-btn')) {
                selectedStoreList = selectedStoreList.filter(function (store) {
                  return store.id !== e.target.closest('li').id;
                });
                (0, _renderSelectedStoreList["default"])(selectedStoreList);
                targetId = e.target.closest('li').id;
                $addStore = document.querySelector("li.item-".concat(targetId, " .add-store"));
            ...
            case 8:
              _context2.prev = 8;
              _context2.next = 11;
              return _firebase.db.collection('votes').doc(params);
            case 11:
              voteItem = _context2.sent;
              _context2.next = 14;
              return voteItem.update({
                stores: (_firebase$firestore$F = firebase.firestore.FieldValue).arrayUnion.apply(_firebase$firestore$F, _toConsumableArray(selectedStoreList))
              });
            case 14:
