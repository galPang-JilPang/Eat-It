...
const updateDOM = (parentNode, realNode, virtualNode) => {
  ...
  if (
    realNode.nodeType === Node.TEXT_NODE &&
  ...
  if (
    realNode.nodeType === Node.COMMENT_NODE ||
    virtualNode.nodeType === Node.COMMENT_NODE
  )
    return;
  ...
  ["checked", "value", "selected"].forEach((key) => {
    if (
      realNode[key] !== undefined &&
      virtualNode[key] !== undefined &&
...
const applyDiff = (realDOM, virtualDOM) => {
  ...
  const [realNodes, virtualNodes] = [
    [...realDOM.childNodes],
    [...virtualDOM.childNodes],
  ];
